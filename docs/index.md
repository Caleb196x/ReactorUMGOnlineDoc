...
| 实践开发 | 组件化、热重载验证、编辑器实时预览、使用`RU_`/`ERU_`UI资产前缀区分运行时与编辑器。 |
| 动画支持 | 支持加载Spine动画文件。 |
...
- **现状**：现在借助Figma make等AI设计工具，我们已经能够实现从自然语言描述->设计稿->React代码的前端UI自动化生产流程，而UE中的UI开发流程仍高度依赖人工，导致效率瓶颈明显。与AI相比，当前UE所提供UI工具的效率差异，差不多类似于传统马车与现代火车之间的差异。
